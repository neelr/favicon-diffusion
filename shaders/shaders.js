...
@builtin(local_invocation_id) local_id : vec3 < u32>)
{
    ...
    if (local_id.x == 0u)
    {
        mpute variance as the mean squared deviation.
        let variance : f32 = sharedSum[0] / f32(rowWidth);
        e addition of eps inside sqrt prevents division by zero (and is how PyTorch applies eps).
    ...
    while (i < rowWidth)
    {
        ...
        ply the affine transform: norm * (1 + scale[i]) + shift[i]
